...
architecture rtl of SD_Read is
    type state_type is (s_idle, s_cmd16, s_cmd16_confirmation, s_cmd17, s_cmd17_confirmation, s_data_read, s_done);
    ...
    constant CMD17_raw : std_logic_vector(7 downto 0) := x"51"; -- READ_BLOCK, Reads a block the size determined by cmd16
    signal address : std_logic_vector(31 downto 0) := x"00100000"; -- 4 byte address to be read
    ...
    constant response_cmd17 : std_logic_vector(7 downto 0) := x"01";
begin
    sck  <= sck_i;
    mosi <= mosi_i;
    cs   <= cs_i;
                            ...
                            if sck_i = '1' then
                                ...
                                if bit_cnt = 0 then 
                                    ...
                                    if byte_buf = response_cmd16 then
                                        cs_i <= '1';
                                        cmd_byte_buf <= CMD17_raw & address & x"00";
                                        state <= s_cmd17;                                                                       
                                    else 
                                        cmd_byte_buf <= CMD16;
                                        state <= s_cmd16;
